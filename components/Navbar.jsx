...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <div className="top-bar hidden lg:block">
        <div className="container flex items-center justify-between">
          <div className="flex items-center gap-6">
            ...
            <a href="tel:+971585521253"
               className="flex items-center gap-1.5 text-white/70 hover:text-white transition-colors text-xs">
              <Phone size={11} /> +971 58 552 1253
            </a>
            <a href="mailto:info@alugridx.com"
               className="flex items-center gap-1.5 text-white/70 hover:text-white transition-colors text-xs">
              <Mail size={11} /> info@alugridx.com
            </a>
          </div>
          <span className="text-white/40 text-xs">Building No.144, Al Jurf 3, Ajman, UAE</span>
        </div>
      </div>
        ...
        <div className="container">
          <div className="flex items-center justify-between h-[68px]">
            ...
            <Link href="/" className="flex items-center gap-2 group flex-shrink-0">
              {/* Logo image — 48×48 so it reads clearly in the 68px header */}
              ...
              <div className="flex flex-col justify-center leading-none">
                <span
                  className="font-heading font-extrabold tracking-widest text-navy"
                  style={{ fontSize: '1.25rem', letterSpacing: '0.12em' }}
                >
                  ALUGRIDX
                </span>
                <span className="text-muted font-body font-normal" style={{ fontSize: '0.6rem', letterSpacing: '0.18em' }}>
                    ...
                    <div className="absolute top-full left-0 w-56 bg-white border border-gray-100 rounded-lg shadow-xl py-1.5 z-50">
                      {item.children.map((child) => (
                        <Link
                          key={child.href}
                          href={child.href}
                          className="block px-4 py-2.5 text-sm text-dark hover:bg-gray-50 hover:text-navy transition-colors font-body"
                        >
                          {child.label}
                        </Link>
                      ))}
      ...
      {/* ── Mobile menu ──────────────────────────────────── */}
      {mobileOpen && (
        <div className="fixed inset-0 z-40 bg-white overflow-y-auto" style={{ paddingTop: '68px' }}>
          <div className="container py-4">
            ...
            <div className="pt-6 pb-8 space-y-3">
              ...
              <a href="tel:+971585521251" className="btn-outline w-full justify-center">
                Call +971 58 552 1251
              </a>
            </div>
          </div>
        </div>
